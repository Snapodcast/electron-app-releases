...
function Nuts(opts) {
    ...
    this.router.post('/ping', function(_req, res) {
        res.send('pong');
    })
}
...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        var url = urljoin(fullUrl, gitFilePath, '/download/version/'+latest.tag+'/'+platform+'?filetype='+filetype);
        res.status(200).send({
            "url": url.indexOf('https://') === -1 ? url.replace('http://', 'https://') : url,
            "name": latest.tag,
            "notes": releaseNotes,
...
Nuts.prototype.onServeVersionsFeed = function(req, res, next) {
    ...
    .then(function(versions) {
        _.each(versions, function(version) {
            var link = urljoin(fullUrl, '/../../../', '/download/version/'+version.tag);
            feed.addItem({
                title: version.tag,
                link:  link.indexOf('https://') === -1 ? link.replace('http://', 'https://') : link,
